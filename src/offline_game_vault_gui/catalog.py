...
def profile_mode(profile: dict[str, Any]) -> MaterializationMode:
    ...
    if platform == "linux" and adapter in {"wine", "bottles", "umu"}:
        if adapter == "wine":
...
def profile_backend(profile: dict[str, Any]) -> BackendId:
    ...
    if platform == "linux" and adapter == "wine":
        return "direct-wine"
    if platform == "linux" and adapter == "umu":
        return "umu"
    if platform == "windows" and adapter == "windows":
        return "windows"
    return "base"
...
def _parse_profile(
    ...
    default_runner = _default_runner_id(
        value,
        runner_object_ids,
        context,
    )
...
def _parse_capsule(path: Path) -> GameRecord:
    if path.is_symlink() or not path.is_file():
        raise CatalogError(
            f"{path}: capsule.json must be a regular file"
        )
    try:
    ...
    if path.parent.name != capsule_id:
        raise CatalogError(
            f"{path}: capsule_id does not match its directory"
        )
    ...
    if (
        content_status_path.is_file()
        and not content_status_path.is_symlink()
    ):
        try:
...
def scan_catalog(
    ...
    if capsules_root.is_symlink() or not capsules_root.is_dir():
        raise CatalogError(
            "A regular 02_CAPSULES directory does not exist"
        )

    games: list[GameRecord] = []
    warnings: list[str] = []
    for candidate in sorted(
        capsules_root.iterdir(),
        ...
        try:
            games.append(
                _parse_capsule(candidate / "capsule.json")
            )
